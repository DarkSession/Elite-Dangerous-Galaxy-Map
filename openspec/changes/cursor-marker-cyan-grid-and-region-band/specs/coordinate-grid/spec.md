...
**Reason**: The merge no longer mixes the line toward the background's own colour. Two of
this requirement's scenarios read that tint under their own names: "The line takes the
background's hue over the core" and "The weight and the tint are one rule". A delta cannot
drop or rename a scenario, so the requirement is replaced by "The grid blends under the
overlays by weight and darkening".

**Migration**: Read "The grid blends under the overlays by weight and darkening". The draw
order, the alpha blend with no depth test, the `smoothstep(0.08, 0.55, L)` merge, the
background reading at a sixteenth of the frame on each axis, the `gridLevels()` rule and the
antialiasing are all carried into it unchanged. What changes is the line's colour, the floor
on its alpha, and the replacement of `gridBackgroundTint` by a colour that darkens toward
`rgb(16, 74, 120)`.

### Requirement: The grid carries coordinate labels

**Reason**: The labels now lie on the grid's own plane and carry all three coordinates, so
the separate plane label at the lower edge of the canvas is gone with its scenario, "The
plane label reads the cursor's height". A delta cannot drop a scenario, so the requirement is
replaced by "The grid carries coordinate labels on its own plane".

**Migration**: Read "The grid carries coordinate labels on its own plane". The alpha gate of
0.09, the local-rate reading the gate takes, the model bounds drop, the line factor of
`alpha / 0.45`, the overlap rule, the switch rule and the page's label readings are all
carried into it. A host that read the element `gm-grid-plane-label` SHALL read any crossing
label and take the second of its three numbers.
