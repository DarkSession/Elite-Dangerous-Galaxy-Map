...
**Reason**: The requirement states that a label does **not** take the range fade, and it
holds two scenarios whose own names read the close zoom band this change deletes.
"The region the camera is inside is named at every zoom" reads a label at full opacity at a
zoom of 10,000 light years, where the boundary beside it draws nothing. "The sweep does not
run below the band" reads the sweep as skipped at a zoom of 4,000 light years, which the new
gate runs, because at a pitch of 35 degrees that frame still holds plane points out to about
26,300 light years. A delta cannot drop or rename a scenario, so the requirement is replaced.

**Migration**: Read "A region in view carries a label that fades with its own range". It
carries every **placement** rule of this requirement without a change: the sampling, the 1
per cent threshold and its half-threshold hysteresis, the plane-space anchor, the centroid
target and the ring search, the box rule, the smoothing rates, `filterAnchor` and the overlap
rule. Two things are not placement rules and both change:

- **the opacity.** The close end of the zoom band goes, and a range fade read at the label's
  own anchor takes its place. A label whose opacity reads 0 is left out of the overlay;
- **the sweep's skip gate.** The zoom half stays and the floor of 5,000 light years is
  replaced by the greatest range to the plane the frame holds. The scenario "The sweep does
  not run when the frame can carry no label" reads the new gate, in place of "The sweep does
  not run below the band".

The flow-field walk this change adds is in the new requirement as well.

### Requirement: The boundaries draw as one soft band inside a zoom band

**Reason**: The pass no longer blurs, so three of this requirement's scenarios read a stage
that is gone: "The blurred band is wider than the unblurred one", "The far band is wider than
the unblurred one as well" and "The blur keeps a straight run and rounds a corner". A delta
cannot drop a scenario from a requirement it modifies, so the requirement is replaced by "The
boundaries draw as one wide soft band". The name also stops claiming a zoom band at the close
end, which the range fade has held per pixel since the change before this one.

**Migration**: Read "The boundaries draw as one wide soft band". Every other rule of this
requirement is carried into it unchanged, including **both fades and all four of their
figures**: nothing under 10,000 light years of range, full at 20,000, full at a zoom of
20,000 and nothing at 30,000. Also carried: the tone, the opacity, the coverage ramp, the
`MAX` join rule, the full-resolution single-channel coverage buffer, the plane-point range
reading, the four view searches with their two premises, `regionCoverageSize()` and the cost
bound. Every scenario is carried except the three named above.

What changes is the **half width**, from a fixed 3 CSS pixels to 1.6 per cent of the viewport
height clamped to 8 and 24, and what goes is the blur stage, the normalisation by the
kernel's peak, the radius table and the corner tolerance of 3 per cent the blur needed.

### Requirement: The region overlay has three modes

**Reason**: The default mode changes from `simplified` to `accurate`, so the scenario "The
default mode is simplified" reads the wrong answer under its own name. A delta cannot drop
or rename a scenario, so the requirement is replaced by "The region overlay has three modes
and starts on the traced set".

**Migration**: Read "The region overlay has three modes and starts on the traced set". The
three values, the options field, `getRegionMode()`, `setRegionMode(mode)`, the rule that a
bad value changes nothing and the `regions` pass switch are all carried into it unchanged. A
host that relied on the map starting in `simplified` SHALL pass `regionMode: 'simplified'`
in its options.
