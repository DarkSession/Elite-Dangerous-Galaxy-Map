...
- [ ] 1.1 In `.devcontainer/post-create.sh`, give `node` the directory
...
- [ ] 1.2 In the same script, install the Firefox browser and its system libraries beside
      Chromium. Verify with `npx playwright install --dry-run firefox`, which must name a
      path under `/home/node/.cache/ms-playwright`
- [ ] 1.3 Add the Firefox section to `.devcontainer/README.md`: no GPU flags, the
...
- [ ] 1.4 Add `tests/devcontainer.test.ts`, which reads `post-create.sh` and asserts the
...
- [ ] 2.1 In `src/app/grid-labels.ts`, replace `GRID_LABEL_SHADOW` with the stroke
      ...
      label
- [ ] 2.2 In `src/app/markers.ts`, replace the name label's `textShadow` with the same
      pair at 2 pixels and `rgba(0, 0, 0, 0.9)`. Verify with `pnpm test`
- [ ] 2.3 Update the browser test that reads the label look so it asserts `none` for the
      ...
      Chromium timed pass
- [ ] 2.4 Take a screenshot of a coordinate label and a marker name label over the
...
- [ ] 3.1 In `src/hud/styles.ts`, remove all five `backdrop-filter` pairs. Raise the
...
- [ ] 3.2 Add the three browser scenarios of `map-hud`: no element under the HUD root
...
- [ ] 3.3 Take a screenshot of the HUD before and after, at the same view, and compare
...
- [ ] 4.1 In `src/hud/dom.ts`, take `formatCoordinate` to 5 decimal places and rewrite the
...
- [ ] 4.2 Update the two browser scenarios of `map-hud` that read the position text, and
      the note in "The distance fields stay whole" that quotes `-9,530.938`. Verify with
      the Chromium pass

## 5. The Firefox project and the budget

- [ ] 5.1 Add a `firefox` project to `playwright.config.ts`. It takes
...
- [ ] 5.2 Make the Firefox project run in the timed pass of `scripts/e2e.mjs`, on one
      worker, beside `chromium-timed`. Verify by running `pnpm test:e2e` and reading the
      two pass headers in the output
- [ ] 5.3 Write `e2e/paint-cost.spec.ts`: open the page **with the HUD on**, which
...
- [ ] 5.4 Add the two regression scenarios to the same spec. Each one adds a **stylesheet
...
- [ ] 5.5 Extend `tests/devcontainer.test.ts`, or add `tests/browser-suite.test.ts`, to
...
- [ ] 6.1 Update `docs/roadmap.md` at the two decisions this change moves: the browser
...
- [ ] 6.2 Add Firefox to the README's "Hardware rendering" section and to the browser suite
...
- [ ] 7.1 Run `pnpm lint`, the TypeScript check and `pnpm test`. All three pass
- [ ] 7.2 Run `pnpm test:e2e` end to end, one run only, and read both passes. Every spec
...
- [ ] 7.4 Run the `openspec-implementation-reviewer` subagent on the change and act on its
