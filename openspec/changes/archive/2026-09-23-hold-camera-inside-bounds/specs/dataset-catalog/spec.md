...
**Reason**: The rule that holds the camera loses its fifth condition, and with it the scenario
"A camera zoomed in closer than the frame is framed again". OpenSpec cannot drop one
scenario from a MODIFIED requirement, so the requirement is replaced in whole.

**Migration**: The requirement "The handle loads one dataset at a time and holds a camera
inside the new bounds" below holds the same text and the same scenarios, with four
conditions in place of five. No host code changes.
