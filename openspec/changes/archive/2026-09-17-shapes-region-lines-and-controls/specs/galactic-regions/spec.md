...
**Reason**: The flow field rides in `RegionLinesResponse`, which now carries one boundary set
and not two, so this requirement's body and its scenario "The field is sent with the boundary
sets" both read a message shape that is gone. A MODIFIED block replaces a requirement but
cannot drop one of its scenarios, so the rewrite is written as a removal and an addition. The
replacement is the added requirement "The region worker builds a flow field toward each
region's centre and sends it with the boundary set", which carries every rule of this one.

**Migration**: None for a host. The flow field is built inside the map and no part of it
reaches the public surface.

### Requirement: The boundary sets are built from one trace of the region grid

**Reason**: The build emitted two boundary sets and now emits one. The smoothed set is gone
with the `simplified` region mode that drew it, so the five scenarios that read it go with
it: "The drawn line stays near the boundary", "The traced set keeps the corners the smoothed
set removes", "The drawn line reads as a line", "The drawn line carries no visible corner"
and "The set is small enough to upload once". A MODIFIED block replaces a requirement but
cannot drop one of its scenarios, so the rewrite is written as a removal and an addition. The
replacement is the added requirement "The boundary set is built from one trace of the region
grid", which carries every rule of this one that the traced set holds.

**Migration**: None for a host. The boundary set is built inside the map and no part of it
reaches the public surface.


### Requirement: The region overlay has three modes and starts on the traced set

**Reason**: The three modes are replaced by one switch, which the added requirement "The
region overlay has a host switch and starts on" states. The `simplified` mode drew the smoothed
boundary set. The two sets now sit 15.7 light years apart at most, which is 1.5 CSS pixels at
a range of 10,000 light years against a band 34.6 CSS pixels wide, so the mode moved the line
by a twentieth of the band and the user could not read the difference. The traced set is also
the nearer of the two to the data, the smaller, and the only one that keeps a corner of the
region data. Nothing is left for the second set to offer, and it cost 805 KiB of `float32`
positions across the worker boundary and to the card on every load.

**Migration**: `GalaxyMapOptions.regionMode` is gone. A host that passed `'off'` SHALL pass
`regions: false`. A host that passed `'accurate'` or `'simplified'` SHALL drop the option,
because on is the default. `getRegionMode()` becomes `areRegionsVisible()`, and
`setRegionMode(mode)` becomes `setRegionsVisible(mode !== 'off')`. The exported type
`RegionMode` is gone, which `library-package` states. This is a breaking change to the
published surface, and the package is at version 0.1.0.
