...
- [x] 1.1 Read `reclampView` in `src/app/create-map.ts` and confirm it ends the glide on
...
- [x] 1.2 Build the demo site once, then run the two failing tests ten times each and
...
- [x] 2.1 Rewrite the body of `narrowing the bounds drops a running zoom glide` in
...
- [x] 2.2 Assert, in this order: the target read before the call is not null; the derived
...
- [x] 2.3 Rewrite the comment above the test to name the spec scenario "Narrowing the
...
- [x] 2.4 Run the test 10 times and verify 10 of 10 pass:
      `GALAXY_MAP_E2E_BUILT=1 pnpm exec playwright test --project=chromium-gpu -g "narrowing the bounds drops a running zoom glide" --repeat-each=10 --workers=1`
- [x] 2.5 Negative control: comment out the `glideOutside` clause of `reclampView`, run the
...
- [x] 3.1 Change `addRule` in `e2e/paint-cost.spec.ts` to take an id beside the rule text
      ...
      passes
- [x] 3.2 Add a helper that runs four pairs of moves, each pair one flat move and one
...
- [x] 3.3 Add `PANEL_RISE_MS = 1.5` beside `RISE_MS`, with a doc comment that states the
...
- [x] 3.4 Rewrite `the blurred panel backdrop fails the budget` to take the four pairs and
...
- [x] 3.5 Verify the label shadow test still reads one pair and `RISE_MS`, and that
      `GALAXY_MAP_E2E_BUILT=1 pnpm exec playwright test --project=firefox -g "the blurred label shadow" --workers=1`
      passes

## 4. The documents

- [x] 4.1 Update the file comment at the head of `e2e/paint-cost.spec.ts` to state the
      paired reading, the turned-around second pair, and why the rise is a median
- [x] 4.2 Add each reading to the `docs/roadmap.md` section of the work it belongs to: the
...
- [x] 4.3 Verify `openspec validate fix-the-two-flaky-browser-tests --strict` reports the
      change valid
...
- [x] 5.1 Verify the tree is otherwise final: `pnpm lint` and `pnpm exec tsc --noEmit` are
...
- [x] 5.2 Measure the new statistic 12 times and record every median, every flat mean,
...
- [x] 5.3 Set the floor from the rule the spec states: at most the smallest of the 12
...
- [x] 5.4 Where the sample standard deviation of the 12 medians is not below **0.325 ms**,
      ...
      **The trigger fired and the fallback was not taken.** The 12 medians gave
      **0.3270 ms**, which is not below 0.325 ms, so this task called for one pair and a
      floor of 1.2 ms. The owner read the numbers and chose to keep the four pairs and the
      1.5 ms floor. The 0.325 ms comes from 22 readings of three other trees. The
      like-for-like reading of this tree is 0.389 ms over 96 single pairs against 0.341 ms
      over 24 medians, which is 12 per cent smaller. `design.md` records the decision and
      `specs/browser-suite/spec.md` records both spreads.
- [x] 5.5 Write the readings into the requirement "A camera move holds the paint budget in
      Firefox" in
...
- [x] 5.6 Run the test 12 more times with the floor that task 5.3 or task 5.4 left, and
      verify 12 of 12 pass

## 6. Verify the whole tree

- [x] 6.1 Run `pnpm lint` and `pnpm exec tsc --noEmit` again and verify both are clean
- [x] 6.2 Run `pnpm test` and verify it passes, with `tests/browser-suite.test.ts` among
      the files it ran
- [x] 6.3 Run `pnpm test:e2e` to the end, with no other Playwright run on the machine, and
      verify both passes are green
- [x] 6.4 Mark every task above complete, then run the `openspec-implementation-reviewer`
