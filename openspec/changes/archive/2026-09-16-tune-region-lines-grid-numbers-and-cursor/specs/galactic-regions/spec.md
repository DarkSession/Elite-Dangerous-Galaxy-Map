...
**Reason**: The band is no longer one tone with a ridge profile. Its scenario "The line is
one tone and lightens what it crosses" names the rule that goes, and its reading that no
pixel of the band is darker than the frame without it is a rule the deeper outer tone
breaks over the bright disc. A delta cannot drop or rename a scenario of a requirement it
modifies, so the requirement is replaced.

**Migration**: Read "The boundaries draw as a wide band with a light core over a smoothed
line". It carries every other rule of this requirement without a change: the plane, the
camera-relative draw, the set each mode draws, the half width and its clamp, the coverage
buffer and its `MAX` blend, the smoothed line, the range fade, the zoom fade, the searched
views and their counts, the cost bound and `regionCoverageSize()`. Three of its scenarios
restate a reading that follows the profile across the band and not the band's own width:
"The band is the stated share of the viewport" reads 30.6, 20.2 and 14.0 CSS pixels at half
maximum, against the outer plateau, in place of 17.3, 11.5 and 8.0 against the peak; "A join is not brighter than the line" allows one
8-bit step in place of the half pixel sampling loss; and "The corner of the traced set is
round to the band's half width" reads against the outer plateau and adds 2.0 CSS pixels in
place of doubling. The band's own half width, 17.28 CSS pixels at 1,080 rows, does not move
in any of them.
