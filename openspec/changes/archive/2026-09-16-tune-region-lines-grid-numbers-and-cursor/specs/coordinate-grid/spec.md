...
**Reason**: The zoom reach goes, and with it the exemption of the numbered level. Six of
the requirement's scenarios read that bound and cannot be corrected: "A level fades out at
the zoom reach", "The reach is the lesser of the two", "The level's own reach still binds
where it is the smaller", "The numbered level keeps its own reach", "The lattice marks the
same part of the frame at every zoom" and "The lattice stops at its reach and the numbered
level goes on". A delta cannot drop a scenario from a requirement it modifies, so the
requirement is replaced.

**Migration**: Read "The grid draws every decade level to its own reach on the cursor's
plane". It carries every other rule of this requirement without a change: the plane at the
cursor's `y`, the six decade levels, the width, the alpha and the boldness by screen
spacing, the reach of 100 of a level's own lines, the camera distance band, the cyan
colour, the larger of two alphas at a crossing and the stop at the model bounds. It keeps
the nine scenarios that do not read the zoom bound and adds two in place of the six that
do: "The lattice runs to the edge of the frame", which reads the far side of the frame at a
pitch of 89 degrees, and "The lattice draws past the old zoom bound at a shallow pitch",
which reads the near side at a pitch of 45.
