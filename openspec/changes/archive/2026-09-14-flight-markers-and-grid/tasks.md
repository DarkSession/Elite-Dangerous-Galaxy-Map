...
- [x] 1.1 Measure the fill risk first: add a temporary browser test that puts 10,000 glow
...
- [x] 1.2 Rewrite `markerCssSize` in `src/scene-data/marker-size.ts` as the four-stop
...
- [x] 1.3 Add the unit test for the curve's shape: 1,000 ranges spaced evenly in the
...
- [x] 1.4 Send the stop table to the marker pass as `uSizeRanges` and `uSizeValues`, and
...
- [x] 1.5 Update `src/scene-data/picking.ts` so the pick radius reads the range alone, and
...
- [x] 1.6 Update `src/app/markers.ts` to call `markerCssSize` without `focalCss`. Verify
      `src/app/markers.test.ts` passes and the ring, the pin offset and the label offset
      all follow the new size
- [x] 1.7 Update the browser tests that put a marker "at the cap": in `e2e/systems.spec.ts`
...
- [x] 1.8 Add the browser test for the viewport rule: one system 4,000 light years from
      the camera, read the row width at 1,080 CSS rows and at 400. Verify the two counts
      agree within 1 and each is 9 within 1
- [x] 1.9 Add the browser test for the pick radius and the viewport: the largest whole
      pixel offset that still picks is the same at 1,080 and at 400 CSS rows
- [x] 1.10 Re-run the marker frame budget scenarios of `e2e/frame-budget.spec.ts` with the
...
- [x] 2.1 Add `src/camera/flight.ts` with `FLIGHT_MS`, the ease `1 - (1 - u)^3` and
...
- [x] 2.2 Hold the flight state in `src/app/create-map.ts`: the start view, the end view
...
- [x] 2.3 Read `prefers-reduced-motion` at each selection and write the end state in one
...
- [x] 2.4 Add `endFlight()` and call it from the pointer, wheel and key handlers and from
      ...
      it
- [x] 2.5 Add `debug.selectionFlightMs()` and its hook in `src/app/main.ts` and
      `e2e/global.d.ts`. Verify it reads above 0 during a flight and 0 before and after
- [x] 2.6 Put `test.use({ reducedMotion: 'reduce' })` on the `describe` blocks of
...
- [x] 2.7 Add the flight tests in a `describe` of their own in `e2e/selection.spec.ts`, with
...
- [x] 2.8 Add the frame rate reading: 10,000 systems, a flight from 20,000 light years at
...
- [x] 3.1 Rewrite the level rule in `src/render/grid-pass.ts`: the six decade levels, the
      ...
      six readings
- [x] 3.2 Add the label level rule: the smallest of the six levels at least 400 CSS pixels
...
- [x] 3.3 Add the per-level camera phase: the camera's `x` and `z` modulo each level's
...
- [x] 3.4 Write the geometry half of the new shaders: `grid.vert` emits a full-screen
...
- [x] 3.5 Add the level loop to `grid.frag`: for each of the six levels the distance to the
...
- [x] 3.6 Rewrite `createGridPass` to draw one call of 3 vertices with the new uniforms,
      ...
      `drawArrays` of 3
- [x] 3.7 Replace `debug.gridPlanes()` with `debug.gridLevels()`, point `gridSpacingLy()`
...
- [x] 3.8 Add `src/app/grid-labels.ts`: the 289 candidate crossings, the projection, the
      ...
      overlap skip
- [x] 3.9 Call the grid label overlay from the frame loop after the marker overlay, and
...
- [x] 3.10 Rewrite the geometry tests of `e2e/grid.spec.ts`: the constant game coordinate,
      ...
      passes
- [x] 3.11 Rewrite the look tests of `e2e/grid.spec.ts`: the coarse line bolder and
...
- [x] 3.12 Rewrite the switch and probe tests of `e2e/grid.spec.ts`: the grid off by
      default, the switch reaching the next frame, and the probes empty with the grid off.
      Verify each passes
- [x] 3.13 Look at the frame at a pitch of 5 degrees and confirm the grid fades into the
      distance rather than ending at a line, which is the risk `design.md` names. Report
      what the frame shows
- [x] 3.14 Read the grid frame budget with `measureFrames` at a pitch of 5 and of 89
...
- [x] 3.15 Verify the committed baseline image still matches, because the grid is off
      unless the options ask for it

## 4. The demo data set

- [x] 4.1 Add `data/` to `.gitignore` and add `scripts/build-demo-systems.mjs`, which
      ...
      accepts
- [x] 4.2 Write the conversion as an exported function of
...
- [x] 4.3 Commit the generated `src/app/demo-systems.json` and add
      ...
      rejects none
- [x] 4.4 Commit a fixture extract of about 20 sites over 8 systems in `tests/fixtures/`
      ...
      network
- [x] 4.5 Change `e2e/hud.spec.ts` "a demo picture loads from the built page" so it stops
      ...
      demo set
- [x] 4.6 Update `README.md`, `THIRD_PARTY_NOTICES.md` and the comment at
...
- [x] 4.7 Open the dev page and confirm the information panel shows the thumbnails from
...
- [x] 5.1 Add this phase to `docs/roadmap.md`: the flight, the size curve, the grid levels,
