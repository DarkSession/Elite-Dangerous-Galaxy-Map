...
needs sets above the bound `raise-the-system-set-bound` raised, which is archived. The
multi-page build this change adds is what that page is built on.
...
- **A cycle that converts to no record is left out of the catalog and named in the build
  output.** Every cycle of the archive gives at least one record today, so the rule writes
  nothing out. It holds for an archive that grows, because an empty entry would give the
  dialog a week with nothing to draw.
...
- A set holds at most `MAX_SYSTEMS` systems, which the package exports and `real-systems`
  states, and which reads **50,000** today. The largest cycle measured gives **1,269**
  records, so every cycle is inside the bound with room over it.
...
- **The committed data has a stated ceiling.** `apps/demo/demo-data/cycles/` stays under
  **25 MB** and one cycle under **1 MB**. The measurement behind it: the largest cycle
  source file, 12.4 MB, converts to 216 KB of JSON, and 110 cycles are 11.5 MB. A test
  reads both sizes.
...
| `apps/demo/scripts/build-demo-systems.mjs` | `Recovery` joins the war states the conversion keeps         |
...
| `README.md`                              | The seven example blocks go. A list of the nine samples replaces them |
| `AGENTS.md`                              | The new pages, and the script counts, which move to 6 and 7    |
