...
**Reason**: The catalog holds six sets, and every shape of the two shape sets now names a
category. The requirement "The demo site carries six data sets" replaces it whole and
keeps every rule the five sets already hold.

**Migration**: None for a host. The demo site is the only caller, and the sixth entry adds
itself to the array the page already gives `datasets`.
