...
- The dataset catalog is a library capability. The page is a host that passes `datasets`
  and `dataset`.
- **This change follows the multi-page demo build** of `publish-the-sample-pages`, and
  `raise-the-system-set-bound`, which took the system set bound to 50,000 and measured the
  renderer, the pick and the switch at it. Both are archived.
...
| Clouds converted, minified JSON, measured            | 10.20 MB over 25 parts, 1.882 keys a record               |
...
# This probe joined the descriptions of a system's rows and read 14.03 MB. The build
# keeps the first description instead, because a joined string composes text the source
# does not state per record, which the spec rules out. The build measures 10.20 MB.
...
for each of those five as well. A gap would be five maps a reader of this page cannot find.
The sixth, the notable systems, comes from `data/csvCache/notable_systems.json`, which no
`MapData-*.js` reads, so it is not a Canonn map and this page holds no entry for it.
...
### One script, one table row per map, five readers

`apps/demo/scripts/build-canonn-sets.mjs` holds one row per Canonn map: the identifier, the
label, the subject group, the shape of its source and the addresses that shape needs. Five
readers cover every row:

| Shape      | What it reads                                                     | Maps it covers                                                                                         |
| ---------- | ----------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------ |
| `literal`  | The `systemsData` literal in the `MapData-*.js` file              | Adamastor, Permit and UIA                                                                              |
...
| `repoData` | A data file of the Canonn source tree, read from `raw.githubusercontent.com` | Cartographics, listening posts, gnosis, the 8 UIA waypoint tables and the other 14 maps that read `Source/data/` |

_Alternative rejected:_ one converter per map, 49 of them. The maps repeat five shapes, and
...
so the worker posts every record of the largest entry, 47,331 after the splits, and the
main thread pays a structured clone it did
...
is not `multifaction`. The parse of the largest set, 2,837,346 bytes, runs in the worker. The page's `load()`
awaits the worker,
...
- **The codex CSV shape differs per subject** → The `codexCsv` reader is written against
  ...
  dead source is.
- **The map draws up to 47,331 markers in one set**, which is the largest entry after the
  splits → `raise-the-system-set-bound` measured the frame budget, the pick bound and the
  switch budget, and landed the bound at 50,000, which is the number those readings hold.
  This change reads that bound, stays inside it and holds
