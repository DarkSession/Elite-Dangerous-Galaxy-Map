...
The base class block spans 8 boxel edges around the camera, and the base class edge is at
least one thirty-second of the zoom distance, so the rule covers at least one eighth of
the zoom distance in every direction. Beyond that block a boxel is coarser and draws
stars further apart, so an invented star there is not a twin of a real one.
...
#### Scenario: The sweep stays inside the frame budget

- **WHEN** a unit test adds 10,000 systems spread evenly over the base class block, then
  builds the suppressed sets of a new drawn set, and then builds them again after the
  camera moves by one base boxel
- **THEN** the first build takes under 12 ms and the second takes under 2 ms
...
The boxel SHALL draw `min(256, round(count))` stars, less the stars a real system
suppresses. A boxel whose count rounds to zero SHALL draw no star.
...
which is how many stars have a size above zero. The second number SHALL exclude the
stars a real system suppresses, so it never rises when a host adds systems.
...
bounds. Each drawn star SHALL carry that light divided by the number of stars the boxel
draws after suppression. The light of a boxel SHALL NOT depend on the calibration, and
SHALL NOT depend on the real systems inside it, so the count, the light and the real
data are set by separate rules. A boxel that draws a real system therefore keeps the same
light over fewer stars, and the galaxy holds its brightness when a host loads data.

A boxel whose every star is suppressed SHALL carry no light. That needs a real system
within 3 light years of every star of the boxel, which the drawn spacing allows only
where the boxel draws one or two stars, so the light lost cannot show.
...
The count `n` SHALL be the count before suppression, so the field's look does not shift
when a host loads data near the camera. The on-screen size SHALL be held between 1 and 16
pixels.
