...
- [x] 4.1 Write the calibration ramp in `src/scene-data/star-field.ts` on the detailed
...
- [x] 4.2 Add the drawn count capped at 256; verify the scenario "empty space draws no
      star" passes
- [x] 4.3 Compute the integral of `detailedMassDensity` over the model bounds, carry it
...
- [x] 4.4 Add the light per star, the radius by spacing and the brightness rule
...
- [x] 4.5 Add the boxel table builder that writes one record per boxel — the origin
...
- [x] 5.1 Write `src/render/shaders/stars.vert` and `stars.frag`: the hash from the
      ...
      hook
- [x] 5.2 Write `src/render/star-pass.ts` with one
...
- [x] 5.3 Add the handover: the inner radius `3 * edge(s0+2)`, the outer radius
...
- [x] 5.4 Add the `stars` switch to `PassSwitches`, `GalaxyMapGlobal` and
...
- [x] 5.5 Add the `float32` emulation unit test for the drawn position; verify every
...
- [x] 5.6 Verify the browser scenarios "the same view gives the same frame by any
      route", "the far view is unchanged" and "the handover keeps the light" pass

## 6. The region data

- [x] 6.1 Write `src/scene-data/regions.ts`: the 42 region records from
      `astro/codex-region`; verify the unit tests "the region list" and "known
      positions resolve" pass
- [x] 6.2 Write `src/scene-data/region-lines.ts`: the 2,027 by 2,027 grid fill through
...
- [x] 6.3 Write `src/scene-data/region-lines.worker.ts`, add `regionLines` to
...
- [x] 6.4 Verify the browser scenarios "time budget" and "main thread stays responsive"
      still pass with the third worker in the ready gate

## 7. The region overlay

- [x] 7.1 Write `src/render/region-pass.ts` and its shaders: camera-relative lines on
...
- [x] 7.2 Add the fade in from 30,000 to 20,000 light years and the fade out from 3,000
...
- [x] 8.3 Add the candidate rule by region bounds and the anchor: the projected
      ...
      of the inset
- [x] 8.4 Add the placement order, the overlap rule and the cap of 12; verify a unit
      test that a smaller region's label is dropped when it overlaps a larger one's and
      that at most 12 survive
- [x] 8.5 Wire the labels into the frame loop behind the `regions` switch and the same
...
- [x] 9.1 Run `pnpm test`; verify every unit test passes
- [x] 9.2 Run `pnpm lint` and `pnpm build`; verify both pass with no error
- [x] 9.3 Run `pnpm test:e2e`; verify the baseline image test still matches without a
      retake and the scenarios "the far view is unchanged" and "the added passes leave
      the far view alone" pass
- [x] 9.4 Run the frame budget suite at 1920x1080; verify the six close views at 500,
      1,000 and 4,000 light years and the ten existing views all stay under 16.7 ms, and
      record the measured means
- [x] 9.5 Update `docs/roadmap.md`: mark phase 2 implemented, correct the calibration
