...
the core is red". This proposal reads that as: the disc is yellow-orange, and the red
tone belongs to the core's surroundings, not to the arms. The colour scenarios encode
that reading, and the owner corrects it at review if it is wrong.

## What Changes

- The volume ramp gets four stops: a blue-violet haze, tan arms, an orange-red band at
  the edge of the bulge, and a cream-yellow core. The point sprites move to the same
  warm side.
...
- The tone map adds a deterministic dither of one 8-bit step, so smooth gradients do
  not band.
- The glow gets a proper downsample, a narrower blur and a lower weight. The sky above
  the disc at the side view stays near the background. The halo past the rim stays,
  smaller.
...
- `src/render/glow-pass.ts` and `src/render/shaders/blur.frag`: the downsample and
  the constants.
...
half-resolution target, each capped at 128 device pixels on a side, which is 64
target pixels, so its worst-case fill is 41 million target pixels per frame. The cap
is reached below 22,000 light years of zoom distance and the pass is fully on from
12,000, so 12,000 is its worst case, and the frame budget scenario gains that
distance: 16.7 ms mean at 1920x1080 at eight views.
