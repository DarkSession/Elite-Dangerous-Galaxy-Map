...
**This change waits for two others.**

1. `publish-the-sample-pages` adds the multi-page demo build this page is built on.
2. **`raise-the-system-set-bound`** raises the 10,000 system bound of `real-systems` to
   **100,000, or to the highest number its readings hold**. The Canonn data is far above
   the bound that stands: the Clouds map alone holds **71,142 systems**, and one Geology
   codex file holds **30,668**. At 100,000 each of them is one entry. At 10,000 the Clouds
   map alone is eight entries or more, and the page is a picker the user has to reassemble
   in their head. Task 1.2 reads the bound that change lands and writes the entry count the
   split rule gives at that number, so a lower bound makes a longer list and not a blocked
   page.
...
- **A new build script fetches and converts the data.** It reads each map's own source to
  learn what that map draws, then fetches what that map fetches. Four source shapes cover
  the 49: the object literal in the file, the JSON dumps of the `canonn-downloads` bucket,
  the codex CSV dumps of that bucket, and the query endpoints of the Canonn cloud
  functions.
...
- **A source is converted once.** The 49 maps name 115 sources between them and 16 of those
  ...
  tree on its ceiling.
- **The page holds its own set for the six maps the demo page already commits**, because it
  holds one entry per Canonn map and a gap would be six maps a reader cannot find. Both
...
- **The page reads a set in a worker.** `apps/demo/src/multifaction.ts` and its worker are
  ...
  14.03 MB `JSON.parse`. The page opens on a small entry, so nothing large is fetched until
...
| Clouds converted                      | **14.03 MB** of minified JSON, ratio 0.432, 197 B a record             |
...
on 2026-09-21. A dump converts at 0.432 of its source, because one row per signal becomes one record per
...
- **The committed tree stays under 50 MB, and one set under 16 MB.** A megabyte is 1,000,000
  ...
  largest single set, which is 14.03 MB. Task 3.3 reports the measured total against the
  ceiling before the data is committed.
...
- **The page draws up to one set bound of markers**, which is the bound
  ...
  of its own.
- **One set transfers at a time.** The largest is 14.03 MB, which GitHub Pages serves
  gzipped as about 1.70 MB. The parse runs in a worker, so the frame loop keeps drawing while
  the set arrives.
