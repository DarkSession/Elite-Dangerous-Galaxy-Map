...
- **A record SHALL NOT repeat one of its own keys as its description.** The Clouds map keys on
  the description column, so 9,209 of its 71,142 records would carry one string twice. This is
...
**One file may appear twice in one entry.** `MapData-All.js` names
`dumpr/Thargoid/2100101.csv` twice, as `bmsites` and as `tbsites1`, so the file is fetched
once and its records are added under both categories. "A source is converted once" binds the
**committed tree**: no two paths hold one source. It does not bind the manifest.

**A source is converted once into one file, and an entry names the files it reads.** 49 maps name 115
sources between them, and 16 of those sources feed more than one map: `MapData-All.js` alone
names 60 of the 63 codex files, and `Aliens`, `BT`, `Guardians`, `TB` and `Thargoids` repeat
13, 8, 8, 5 and 5 of them. A set file per map would write the codex corpus twice, which is about 6 MB
...
- **WHEN** the browser suite loads the Clouds entry and reads the category rows
- **THEN** the map holds the 35 keys the Clouds map's own reader gives, not one and not the 24
  values of the source's `category` column, and each row carries the name and the colour that
  map gives that key
...
`raise-the-system-set-bound` lands at **100,000**. The Clouds map holds 71,142 systems and
one Geology codex file holds 30,668, so each of them is one entry at that bound. No
measured map splits at 100,000. This requirement is what holds a map that passes the bound
...
holds.** The Clouds map converts to 14.03 MB over 71,142 records, which is 197 bytes a record,
...
copy of it. `raise-the-system-set-bound` exports `MAX_SYSTEMS`, which the build reads from the
built package, and the build SHALL fail where it cannot read it rather than fall back to a
number of its own. It SHALL read the byte ceiling from the same place the ceiling
...
and the address in its output, and SHALL finish. A page is then built from what the sources
still hold.
...
**Six of the 49 maps are already committed for the demo page**, converted by
`build-demo-systems.mjs`: the Guardian ruins, the Guardian structures, the notable systems,
the UIA, the Adamastor and the multifaction spheres. This page SHALL hold its own set for
...
- **WHEN** a unit test runs each of the four source readers over a committed fixture of its
  shape
...
The arithmetic behind the ceiling, measured on 2026-09-21 in the record shape this
requirement states. **It counts each source once, which is what the rule "a source is
converted once" makes true**: 49 maps name 115 sources, 16 of them shared, and a file per map
would write about 47 MB. The 74 distinct sources are 11 JSON dumps of 41.43 MB and 63 codex
CSV files of 21.78 MB. A dump converts at **0.432** of its source, read from the Clouds map:
32.49 MB in, 14.03 MB out, 197 bytes a record, which carries the keys its map's reader gives
...
set off the main thread, and `dataset-catalog` states that rule for that entry. The largest set of this page is 14.03 MB, so a `JSON.parse` on the main thread would hold the frame loop
...
largest set fetches 14.03 MB before the user asks for anything.
