...
**Reason**: The catalog holds seven sets. The seventh is the Thargoid war set, which gives
the icon stack a committed set that carries them. The requirement "The demo site carries
seven data sets" replaces it whole and keeps every rule the six sets already hold.

**Migration**: None for a host. The demo site is the only caller, and the seventh entry
adds itself to the array the page already gives `datasets`.
