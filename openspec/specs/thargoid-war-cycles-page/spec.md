...
in the build output**, with its file name and the number of raw records the file held.
Every cycle of the archive gives at least one record today, so the rule writes nothing out.
It holds for an archive that grows, because an empty entry would give the dialog a week
with nothing to draw. A file that holds records the conversion drops in whole is a signal
...
A set SHALL hold at most `MAX_SYSTEMS` systems, which the package exports and
`real-systems` states, and which reads **50,000** today. The largest cycle measured gives
**1,269** records, so every cycle is inside the bound. A cycle that converts to more records
than `MAX_SYSTEMS` SHALL fail the build and SHALL be named, because the map would reject
the records over the bound and the page would draw a part of that week in silence. The
build SHALL read the bound from the package, not write the number again.

The committed data of the page, with its manifest, SHALL stay under **25 MB**, and one
cycle SHALL stay under **1 MB**. The largest cycle measured converts a 12.4 MB source file
to 1,269 records and 216 KB of JSON, and the whole archive gives 11.5 MB, so the ceiling
holds it with room over it.
...
- **WHEN** a cycle file converts to more records than `MAX_SYSTEMS`
