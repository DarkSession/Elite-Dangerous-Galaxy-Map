...
The Examples section SHALL hold one page per example, and each page SHALL carry a
TypeScript block that a reader can copy. **Every member an example imports from the package
SHALL be a member the package exports**, so an example cannot name a call that was renamed
or removed.
...
- **WHEN** an example imports a name from the package that the entry points no longer
  export, and a developer runs the unit tests
- **THEN** the test fails and names the example page and the member
